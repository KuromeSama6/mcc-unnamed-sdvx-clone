...
namespace Arcade.Compose
{
    ...
    public class AdeGridManager : MonoBehaviour, IMarkingMenuItemProvider
    {
        ...
        private void CalculateBeatlineTimes()
        {
            ...
            HideExceededBeatlineInstance();
            var ts = ArcTimingManager.Instance[AdeTimingEditor.Instance.EditingTimingGroup].Timings;

            for (int i = 0; i < ts.Count - 1; ++i)
            {
                float segment = ts[i].Bpm == 0 ? (ts[i + 1].Timing - ts[i].Timing) : (60000 / Mathf.Abs(ts[i].Bpm) / BeatlineDensity);
                if (segment == 0) continue;
                int primaryCount = 0;
                int n = 0;
                float j = ts[i].Timing;
                while (j < ts[i + 1].Timing)
                {
                    j = ts[i].Timing + n++ * segment;
            ...
            if (ts.Count >= 1)
            {
                float segmentRemain = ts[ts.Count - 1].Bpm == 0 ? (ArcGameplayManager.Instance.Length - ts[ts.Count - 1].Timing)
              : 60000 / Mathf.Abs(ts[ts.Count - 1].Bpm) / BeatlineDensity;
                if (segmentRemain != 0)
                {
                    int primaryCount = 0;
                    int n = 0;
                    float j = ts[ts.Count - 1].Timing;
                    while (j < ArcGameplayManager.Instance.Length)
                    {
                        j = ts[ts.Count - 1].Timing + n++ * segmentRemain;
                        AddBeatlineTiming(j, (primaryCount % BeatlineDensity == 0 ? 0 : (primaryCount % (BeatlineDensity / 2) == 0 ? 1 : (primaryCount % (BeatlineDensity / 4) == 0 ? 2 : 3))));
                        primaryCount++;
                    }
                }
        ...
        private void UpdateBeatline()
        {
            ...
            if (EnableBeatline)
            {
                ...
                {
                    if (!ArcTimingManager.Instance[AdeTimingEditor.Instance.EditingTimingGroup].ShouldRender(((int)t.Timing + offset), 0))
                    {
                        continue;
                    }
                    LineRenderer l = GetBeatlineInstance();
                    l.enabled = true;
                    float z = ArcTimingManager.Instance[AdeTimingEditor.Instance.EditingTimingGroup].CalculatePositionByTiming(((int)t.Timing + offset)) / 1000f;
